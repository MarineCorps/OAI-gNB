                              ...
                              int size)
{
  ...
  uint8_t qfi = 0;
  /* SDAP 헤더가 비활성화된 경우 헤더 파싱 건너뜀 */
  if (entity->enable_sdap) {
    // SDAP 헤더에서 QFI 추출 (첫 바이트의 하위 6비트)
    qfi = buf[0] & 0x3F; // QFI is always the first 6 bits in the first octet
    if (qfi >= SDAP_MAX_QFI) {
      LOG_E(SDAP, "Invalid QFI %d received in SDAP header\n", qfi);
      return;
    }
    // QFI -> DRB 매핑 테이블에서 entity role 확인
